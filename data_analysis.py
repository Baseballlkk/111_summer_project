# This program is mainly used to do data analysis in 111 summer research project
# Author : Yu-Chuan Kan
# Date: Aug 26, 2023
...
class Fourier_trans:
    ...
    def DFT(self, arr):
        self.dim_dft = arr.shape[1]

        self.n_dft = np.matrix(np.linspace(0, self.dim_dft - 1, self.dim_dft))
        self.k_dft = 2 * np.pi * np.matrix(np.linspace(0, self.dim_dft - 1, self.dim_dft)) / self.dim_dft

        self.C_dft = np.cos(self.k_dft.T * self.n_dft) * np.matrix(arr).T / (self.dim_dft)
        self.S_dft = np.sin(self.k_dft.T * self.n_dft) * np.matrix(arr).T / (self.dim_dft)

        return self.C_dft, self.S_dft

    def IDFT(self, C_k, S_k, arr):
        self.dim_idft = round(arr.shape[0])
        self.n_idft = np.matrix(np.linspace(0, self.dim_idft - 1, self.dim_idft))
        self.k_idft = (
            np.matrix(np.linspace(0, self.dim_idft - 1, self.dim_idft)) * 2 * np.pi / self.dim_idft
        )
        self.c = np.cos(self.k_idft.T * self.n_idft[:, : round(self.dim_idft / 2)])
        self.s = np.sin(self.k_idft.T * self.n_idft[:, : round(self.dim_idft / 2)])
        self.C_idft = (self.c * C_k).T
        self.S_idft = (self.s * S_k).T

        return self.C_idft, self.S_idft
...
class power_spectrum(Fourier_trans):
    ...
    def power_coe(self, arr):
        self.C_power, self.S_power = self.DFT(arr)
        self.C_power = self.Nyquist(self.C_power, arr, 1)
        self.S_power = self.Nyquist(self.S_power, arr, 1)

        self.A_power, self.B_power = self.DFT(self.C_power)
        self.a_power, self.b_power = self.DFT(self.S_power)

        self.A_power = self.Nyquist(self.A_power, arr, 0)
        self.B_power = self.Nyquist(self.B_power, arr, 0)
        self.a_power = self.Nyquist(self.a_power, arr, 0)
        self.b_power = self.Nyquist(self.b_power, arr, 0)

        return np.asarray([self.A_power, self.B_power, self.a_power, self.b_power])

    def power_spec(self, arr):
        self.A_spec, self.B_spec, self.a_spec, self.b_spec = self.power_coe(arr)

        self.power_pos = 1 / 8 * (
            np.power(self.A_spec, 2)
            + np.power(self.B_spec, 2)
            + np.power(self.a_spec, 2)
            + np.power(self.b_spec, 2)
        ) + 1 / 4 * (np.multiply(self.a_spec, self.B_spec) - np.multiply(self.b_spec, self.A_spec))
        self.power_neg = 1 / 8 * (
            np.power(self.A_spec, 2)
            + np.power(self.B_spec, 2)
            + np.power(self.a_spec, 2)
...
class reconstruction(power_spectrum):
    ...
    def e_w_trans(self, arr_c):
        ...
        self.imag_east = 1 / 4 * (-1 * self.B_e - self.a_e)
